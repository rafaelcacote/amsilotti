                        ...
                        <form action="{{ route('membro-equipe-tecnicas.update', $membroEquipeTecnica->id) }}" method="POST" class="row g-3 needs-validation" novalidate>
                            ...
                            <div class="col-md-4 mb-3">
                                ...
                                <select class="form-select @error('cargo') is-invalid @enderror" id="cargo" name="cargo" required>
                                    ...
                                    <option value="Engenheiro civil" {{ (old('cargo', $membroEquipeTecnica->cargo) == 'Engenheiro civil') ? 'selected' : '' }}>Engenheiro civil</option>
                                    <option value="estagiario de arquitetura" {{ (old('cargo', $membroEquipeTecnica->cargo) == 'estagiario de arquitetura') ? 'selected' : '' }}>Estagiário de arquitetura</option>
                            ...
                            <div class="col-md-4 mb-3">
                                ...
                                <select class="form-select @error('user_id') is-invalid @enderror" id="user_id" name="user_id" required>
                                    ...
                                    @foreach ($usuarios as $item)
                                        <option value="{{ $item->id }}" {{ old('user_id', $membroEquipeTecnica->user_id) == $item->id ? 'selected' : '' }}>
                                            {{ $item->name }}
                                        </option>
                                    @endforeach
                                </select>
